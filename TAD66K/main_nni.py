...
def validate(opt,
  ...
  print(
      'accuracy: {}, lcc_mean: {}, srcc_mean: {}, validate_losses: {}'.format(
          acc, lcc_mean, srcc_mean, validate_losses.avg))
...
def start_train(opt):
  ...
  torch.optim.Adam(model.parameters(), lr=opt['init_lr'])
  ...
  for e in range(opt['num_epoch']):
    ...
    if ((vsrcc > srcc_best or vacc > vacc_best)) and (vsrcc > 0.48):
      srcc_best = vsrcc
      vacc_best = vacc

      model_savetime = 'DOT_10_17_TAD66K'
      model_name = f"e_{e}_{model_savetime}_vacc{vacc}_srcc{vsrcc}vlcc{vlcc}.pth"
